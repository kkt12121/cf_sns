...
import { FILLTER_MAPPER } from './const/filter-mapper.const';
...
export class CommonService {
  ...
  private parseWhereFilter<T extends BaseModel>(
    ...
    const split = key.split('__'); // [where, id, less_than]
    ...
    if (split.length === 2) {
      ...
      // if (operator === 'between') {
      //   options[filed] = FILLTER_MAPPER[operator](values[0], values[1]);
      // } else {
      //   options[filed] = FILLTER_MAPPER[operator](value);
      // }
